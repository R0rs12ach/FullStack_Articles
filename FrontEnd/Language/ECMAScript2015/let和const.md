...
块级作用域的出现，实际上使得获得广泛应用的立即执行匿名函数（IIFE）**不再必要**了。
...
上面代码表明改变常量的值会报错。**注意，如果是*常规模式*，对常量赋值不会报错，但也是无效的**。
...
上面代码表示，对于const来说，只声明不赋值，就会报错。同样的，*这行命令在**常规模式**下不报错*，但`foo`以后也没法重新赋值了。
...
* let&const声明不会变量提升
* let&const暂时性死区
* ES5存在函数提升，**函数声明都会提升到当前作用域的顶部**
* ES6规定，函数本身的作用域，在其所在的块级作用域之内
* const声明赋值必须一次性完成，同时在严格模式下const声明的常量被修改会报错
* const声明的常量只有在当前作用域有效
* const在声明对象的时候，值对应的是变量的地址，地址不可变，但地址内的数据可以变
* ES6在ES5只有var function的基础上，新增了let、const、import、class四个声明变量的方式
* **var、function声明的全局变量，依旧是全局对象的属性，let、const、class声明的全局变量，不属于全局对象的属性**

*[声明：转载自阮一峰博客，二次增加过一定内容。[原文链接](http://es6.ruanyifeng.com/#docs/let)]*